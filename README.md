...
- `PUT /api/statuses/<id>` — body `{"status":"comment-only|comment-and-connect|skip|done|todo","audience":null}` — `audience` optional: `chro`, `icp`, `compliance`, `influencer`, `watchlist`, `law-firm`. `todo` removes the row.
